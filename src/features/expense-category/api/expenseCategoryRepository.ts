import {
  readCollectionSnapshot,
  readVersionedCollection,
  writeVersionedCollection,
} from "@/lib/persistence";
import { categoryRecordSchema } from "@/features/category/schema";
import {
  createDeterministicCategory,
  findCategoryByName,
  normalizeCategoryName,
  uniqueCategoryNames,
} from "@/features/category/utils";
import type { CategoryInput } from "@/features/category/types";
import { CATEGORY_COLORS, normalizeCategoryColor } from "@/features/category/constants";
import type { ExpenseCategory } from "../types";
import {
  EXPENSE_CATEGORY_STORAGE_KEY,
  EXPENSE_STORAGE_KEY,
} from "@/features/expense/constants";

function legacyNames(): string[] {
  const snapshot = readCollectionSnapshot(EXPENSE_STORAGE_KEY);
  return uniqueCategoryNames(
    snapshot.records.flatMap((record) => {
      if (!record || typeof record !== "object" || Array.isArray(record)) return [];
      const name = (record as Record<string, unknown>).category;
      return typeof name === "string" ? [name] : [];
    }),
  );
}

function save(categories: ExpenseCategory[]): void {
  writeVersionedCollection(
    EXPENSE_CATEGORY_STORAGE_KEY,
    categoryRecordSchema,
    categories,
  );
}

function getAll(): ExpenseCategory[] {
  const snapshot = readCollectionSnapshot(EXPENSE_CATEGORY_STORAGE_KEY);
  if (snapshot.exists) {
    const stored = readVersionedCollection(
      EXPENSE_CATEGORY_STORAGE_KEY,
      categoryRecordSchema,
    );
    const normalized = stored.map((category) => ({
      ...category,
      color: normalizeCategoryColor(category.color, `expense:${category.id}:${category.name}`),
    }));
    if (normalized.some((category, index) => category.color !== stored[index].color)) save(normalized);
    return normalized;
  }

  const names = legacyNames();
  const categories = names.map((name, index) =>
    createDeterministicCategory(
      "expense",
      name,
      CATEGORY_COLORS[index % CATEGORY_COLORS.length].value,
    ),
  );
  save(categories);
  return categories;
}

function ensureNames(names: readonly string[]): ExpenseCategory[] {
  const categories = getAll();
  const additions = uniqueCategoryNames(names)
    .filter((name) => !findCategoryByName(categories, name))
    .map((name, index) =>
      createDeterministicCategory(
        "expense",
        name,
        CATEGORY_COLORS[(categories.length + index) % CATEGORY_COLORS.length].value,
      ),
    );
  if (additions.length === 0) return categories;
  const next = [...categories, ...additions];
  save(next);
  return next;
}

export const expenseCategoryRepository = {
  getAll,
  ensureNames,
  save,
  create(input: CategoryInput): ExpenseCategory {
    const categories = getAll();
    if (findCategoryByName(categories, input.name)) {
      throw new Error("DUPLICATE_CATEGORY");
    }
    const category = {
      ...createDeterministicCategory("expense", input.name, input.color),
      id: crypto.randomUUID(),
    };
    save([...categories, { ...category, color: normalizeCategoryColor(category.color, `expense:${category.id}:${category.name}`) }]);
    return category;
  },
  update(category: ExpenseCategory): void {
    const categories = getAll();
    if (categories.some(
      (item) =>
        item.id !== category.id &&
        normalizeCategoryName(item.name) === normalizeCategoryName(category.name),
    )) {
      throw new Error("DUPLICATE_CATEGORY");
    }
    save(categories.map((item) => (item.id === category.id ? { ...category, color: normalizeCategoryColor(category.color, `expense:${category.id}:${category.name}`) } : item)));
  },
  delete(id: string): void {
    save(getAll().filter((category) => category.id !== id));
  },
};
