  ...
  return <article data-invoice-layout={source.invoiceStyle} className={`mx-auto w-full max-w-[780px] overflow-hidden rounded-xl border bg-white text-slate-900 shadow-sm ${styleClass}`}>
    ...
    <div className={`p-5 sm:p-8 lg:p-10 ${source.invoiceStyle === "Creative" ? "bg-gradient-to-br from-primary/8 via-white to-white" : source.invoiceStyle === "Professional" ? "border-t-8 border-primary" : ""}`}>
      <header className={`flex flex-col gap-6 border-b border-slate-200 pb-7 ${source.invoiceStyle === "Neutral" ? "items-center text-center" : "sm:flex-row sm:items-start sm:justify-between"}`}>
        <div className={`flex min-w-0 items-start gap-4 ${source.invoiceStyle === "Neutral" ? "flex-col items-center" : ""}`}>{source.businessLogo && <img src={source.businessLogo} alt="" className={`size-14 shrink-0 object-contain ${source.invoiceStyle === "Creative" ? "rounded-full" : "rounded-lg"}`} />}<div className="min-w-0"><h2 className="break-words text-xl font-bold sm:text-2xl">{source.businessName}</h2>{source.legalName && <p className="mt-1 text-sm text-slate-500">{source.legalName}</p>}<p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-500">{[source.address, source.phone, source.email].filter(Boolean).join("\n")}</p></div></div>
      ...
      {(source.signatureImage || source.stampImage) && <section className="mt-8 border-t border-slate-200 pt-6"><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Authorized signature</p><div className="relative mt-2 h-28 w-52">{source.signatureImage && <img src={source.signatureImage} alt="Visual business signature" className="absolute bottom-2 left-0 h-20 max-w-44 object-contain object-left" />}{source.stampImage && <img src={source.stampImage} alt="Business stamp" className="absolute bottom-0 left-24 z-10 h-24 w-24 object-contain opacity-90" />}</div></section>}
      {source.legalDisclaimer && <p className="mt-6 text-[10px] leading-4 text-slate-400">{source.legalDisclaimer}</p>}
    </div>
...
export default function InvoicePage() {
  ...
  <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
    <DialogContent className="max-h-[calc(100dvh-1rem)] sm:max-w-2xl">
      <DialogHeader><DialogTitle>Invoice settings</DialogTitle><DialogDescription>Use your own business identity. Uploaded marks are visual assets, not certified electronic signatures.</DialogDescription></DialogHeader>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Default style"><select className="native-control" value={settingsDraft.invoiceStyle} onChange={(event) => setSettingsDraft({ ...settingsDraft, invoiceStyle: event.target.value as InvoiceSettings["invoiceStyle"] })}><option>Creative</option><option>Neutral</option><option>Professional</option></select></Field>
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[calc(100dvh-1rem)] max-w-[calc(100%-1rem)] sm:max-w-3xl"><DialogHeader><DialogTitle>{draft.bookingId ? "Booking invoice" : "Custom invoice"}</DialogTitle><DialogDescription>{draft.bookingId ? "Booking details are prefilled. Changes here do not change the booking." : "A custom invoice does not create a Booking, Payment, or Income record."}</DialogDescription></DialogHeader><div className="space-y-6">
    ...
    <section className="rounded-2xl border border-border p-4 sm:p-5">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Invoice style"><select className="native-control" value={draft.invoiceStyle} onChange={(event) => setDraft({ ...draft, invoiceStyle: event.target.value as Invoice["invoiceStyle"] })}><option>Creative</option><option>Neutral</option><option>Professional</option></select></Field>
      </div>
    </section>
