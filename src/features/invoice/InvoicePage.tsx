...
function InvoicePreview({ invoice, settings, payments }: { invoice: Invoice; settings: InvoiceSettings; payments: ReturnType<typeof usePayments>["payments"] }) {
  ...
  return <article data-invoice-layout={source.invoiceStyle} className={`mx-auto w-full max-w-[780px] overflow-hidden rounded-sm border border-slate-300 bg-white text-slate-950 shadow-sm ${documentEdge}`}>
    ...
    <div className="bg-white">
      ...
      <div className="p-5 sm:p-9 lg:p-11">
        ...
        <dl className={`ml-auto mt-7 max-w-[300px] border-t pt-4 text-sm ${strongRule}`}><div className="flex justify-between gap-6 py-1"><dt className="text-slate-600">Subtotal</dt><dd className="tabular-nums">{formatRupiah(totals.subtotal)}</dd></div>{totals.discount > 0 && <div className="flex justify-between gap-6 py-1"><dt className="text-slate-600">Discount{source.discountMode === "percentage" ? ` (${source.discountValue}%)` : ""}</dt><dd className="tabular-nums">-{formatRupiah(totals.discount)}</dd></div>}{totals.tax > 0 && <div className="flex justify-between gap-6 py-1"><dt className="text-slate-600">{invoiceTaxLabel(source)}</dt><dd className="tabular-nums">{source.taxTreatment === "deducted" ? "-" : ""}{formatRupiah(totals.tax)}</dd></div>}<div className={`mt-2 flex justify-between gap-6 border-t pt-3 text-lg font-black ${strongRule}`}><dt>{modernClassic ? "Grand total" : "Total"}</dt><dd className="tabular-nums">{formatRupiah(totals.total)}</dd></div><div className={`flex justify-between gap-6 py-1.5 ${modernClassic && paid > 0 ? "font-bold text-[#ae3041]" : "text-slate-700"}`}><dt>{modernClassic ? "Down payment" : "Paid"}</dt><dd className="tabular-nums">{formatRupiah(paid)}</dd></div><div className="flex justify-between gap-6 py-1 text-base font-black"><dt>Remaining</dt><dd className="tabular-nums">{formatRupiah(remaining)}</dd></div>{paid > totals.total && <div className="flex justify-between gap-6 py-1 font-semibold text-amber-700"><dt>Overpaid</dt><dd className="tabular-nums">{formatRupiah(paid - totals.total)}</dd></div>}</dl>
