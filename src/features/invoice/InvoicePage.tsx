...
function InvoicePreview({ invoice, settings, payments }: { invoice: Invoice; settings: InvoiceSettings; payments: ReturnType<typeof usePayments>["payments"] }) {
  ...
  const styleClass = source.invoiceStyle === "Professional" ? "border-slate-400" : "border-slate-200";
  const headingClass = "text-primary";
  if (source.invoiceStyle === "Modern Classic") {
    return <article data-invoice-layout={source.invoiceStyle} className="mx-auto w-full max-w-[780px] overflow-hidden rounded-sm border border-slate-300 bg-white text-slate-950 shadow-sm">
      {history.length > 1 && <nav aria-label="Invoice revision history" className="flex flex-wrap items-center gap-2 border-b border-slate-200 bg-slate-50 px-5 py-3"><span className="mr-1 text-xs font-bold uppercase tracking-wider text-slate-600">History</span>{history.map((version) => <a key={version.id} href={`/invoices?invoice=${encodeURIComponent(version.id)}`} className={`rounded-lg border px-2.5 py-1 text-xs font-semibold ${version.id === invoice.id ? "border-primary bg-primary text-primary-foreground" : "border-slate-300 bg-white text-slate-800"}`}>V{version.version} · {version.lifecycle === "Issued" ? version.invoiceNumber : version.lifecycle}</a>)}</nav>}
      <div className="p-5 sm:p-9 lg:p-11">
        <header className="grid gap-6 border-b-2 border-slate-950 pb-6 sm:grid-cols-[1fr_auto] sm:items-start"><div className="flex min-w-0 items-start gap-5">{source.businessLogo && <img src={source.businessLogo} alt="" className="size-20 shrink-0 object-contain" />}<div><h2 className="break-words text-2xl font-black tracking-tight">{source.businessName}</h2>{source.legalName && <p className="mt-1 text-sm font-semibold text-slate-700">{source.legalName}</p>}<p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-700">{[source.address, source.phone, source.email].filter(Boolean).join("\n")}</p></div></div><div className="sm:text-right"><p className="text-3xl font-black tracking-[0.08em] text-slate-950">INVOICE</p><dl className="mt-3 space-y-1 text-xs"><div className="flex gap-4 sm:justify-end"><dt className="font-bold">No.</dt><dd>{source.invoiceNumber}</dd></div><div className="flex gap-4 sm:justify-end"><dt className="font-bold">Date</dt><dd>{formatInvoiceDate(source.invoiceDate)}</dd></div><div className="flex gap-4 sm:justify-end"><dt className="font-bold">Due</dt><dd>{formatInvoiceDate(source.dueDate)}</dd></div></dl></div></header>
        <section className="grid gap-5 border-b border-slate-300 py-6 sm:grid-cols-[0.72fr_1.28fr]"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-700">Bill to</p><p className="mt-2 font-black">{source.clientName}</p><p className="mt-1 break-words text-sm text-slate-700">{[source.clientPhone, source.clientEmail].filter(Boolean).join(" · ")}</p></div>{source.serviceName && <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-700">Service</p><p className="mt-2 font-semibold">{source.serviceName}</p></div>}</section>
        <div className="mt-6 overflow-hidden border-y-2 border-slate-950"><div className="hidden grid-cols-[52px_minmax(0,1fr)_120px_58px_125px] gap-3 border-b border-slate-950 py-3 text-[10px] font-black uppercase tracking-wider sm:grid"><span>No</span><span>Item description</span><span className="text-right">Price</span><span className="text-right">Qty</span><span className="text-right">Total</span></div>{source.lineItems.map((item, index) => <div key={item.id} className="grid gap-2 border-t border-slate-300 py-4 first:border-t-0 sm:grid-cols-[52px_minmax(0,1fr)_120px_58px_125px] sm:gap-3"><p className="hidden text-sm sm:block">{String(index + 1).padStart(2, "0")}</p><div><p className="font-bold">{item.item}</p>{item.description && <p className="mt-1 break-words text-xs leading-5 text-slate-700">{item.description}</p>}</div><p className="text-sm sm:text-right"><span className="font-semibold sm:hidden">Price: </span>{formatRupiah(item.unitPrice)}</p><p className="text-sm sm:text-right"><span className="font-semibold sm:hidden">Qty: </span>{item.quantity}</p><p className="font-bold sm:text-right">{formatRupiah(item.quantity * item.unitPrice)}</p></div>)}</div>
        <div className="mt-6 grid gap-7 sm:grid-cols-[1fr_270px]"><div>{source.paymentInstructions && <section><h3 className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-700">Payment</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{source.paymentInstructions}</p></section>}{source.notes && <section className="mt-5"><h3 className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-700">Notes</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{source.notes}</p></section>}</div><dl className="space-y-2 text-sm"><div className="flex justify-between gap-5"><dt>Subtotal</dt><dd>{formatRupiah(totals.subtotal)}</dd></div>{totals.discount > 0 && <div className="flex justify-between gap-5"><dt>Discount</dt><dd>-{formatRupiah(totals.discount)}</dd></div>}{totals.tax > 0 && <div className="flex justify-between gap-5"><dt>{invoiceTaxLabel(source)}</dt><dd>{source.taxTreatment === "deducted" ? "-" : ""}{formatRupiah(totals.tax)}</dd></div>}<div className="flex justify-between gap-5 border-t border-slate-950 pt-3 text-base font-black"><dt>Grand total</dt><dd>{formatRupiah(totals.total)}</dd></div><div className="flex justify-between gap-5 font-bold text-[#b13745]"><dt>Down payment</dt><dd>{formatRupiah(paid)}</dd></div><div className="flex justify-between gap-5 font-black"><dt>Remaining</dt><dd>{formatRupiah(remaining)}</dd></div></dl></div>
        {source.showSchedules && source.schedules.length > 0 && <section className="mt-8 border-t border-slate-300 pt-6"><h3 className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-700">Schedule</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">{source.schedules.map((schedule, index) => { const start = new Date(schedule.startAt); const end = new Date(schedule.endAt); return <div key={`${schedule.startAt}-${index}`} className="border-l-2 border-slate-950 pl-3 text-sm"><p className="font-bold">{new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "short", year: "numeric", timeZone: "Asia/Jakarta" }).format(start)} · {start.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "Asia/Jakarta" })}–{end.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "Asia/Jakarta" })}</p><p className="mt-1 text-slate-700">{[schedule.label, schedule.location].filter(Boolean).join(" · ")}</p></div>; })}</div></section>}
        {(source.signatureImage || source.stampImage) && <section className="mt-8"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-700">Authorized signature</p><div className="relative mt-2 h-32 w-56">{source.signatureImage && <img src={source.signatureImage} alt="Visual business signature" className="absolute bottom-2 left-0 h-20 max-w-48 object-contain object-left" />}{source.stampImage && <img src={source.stampImage} alt="Business stamp" className="absolute bottom-0 left-12 z-10 h-24 w-24 object-contain" />}</div></section>}
      </div>
  ...
  return <article data-invoice-layout={source.invoiceStyle} className={`mx-auto w-full max-w-[780px] overflow-hidden rounded-xl border bg-white text-slate-900 shadow-sm ${styleClass}`}>
    ...
    <div className={`p-5 sm:p-8 lg:p-10 ${source.invoiceStyle === "Creative" ? "bg-gradient-to-br from-primary/8 via-white to-white" : source.invoiceStyle === "Professional" ? "border-t-8 border-primary" : ""}`}>
      <header className={`flex flex-col gap-6 border-b border-slate-200 pb-7 ${source.invoiceStyle === "Neutral" ? "items-center text-center" : "sm:flex-row sm:items-start sm:justify-between"}`}>
        <div className={`flex min-w-0 items-start gap-4 ${source.invoiceStyle === "Neutral" ? "flex-col items-center" : ""}`}>{source.businessLogo && <img src={source.businessLogo} alt="" className={`size-20 shrink-0 object-contain ${source.invoiceStyle === "Creative" ? "rounded-full" : "rounded-lg"}`} />}<div className="min-w-0"><h2 className="break-words text-xl font-bold sm:text-2xl">{source.businessName}</h2>{source.legalName && <p className="mt-1 text-sm text-slate-700">{source.legalName}</p>}<p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-700">{[source.address, source.phone, source.email].filter(Boolean).join("\n")}</p></div></div>
        <div className="sm:text-right"><p className={`text-2xl font-bold tracking-tight ${headingClass}`}>INVOICE</p><p className="mt-1 text-sm font-semibold">{source.invoiceNumber}</p></div>
      </header>
      ...
      <div className={`${source.invoiceStyle === "Creative" ? "space-y-3" : `overflow-hidden border border-slate-200 ${source.invoiceStyle === "Professional" ? "rounded-none border-x-0" : "rounded-lg"}`}`}>
        {source.invoiceStyle !== "Creative" && <div className={`hidden grid-cols-[minmax(0,1fr)_70px_130px_130px] gap-3 px-4 py-3 text-xs font-bold uppercase tracking-wider sm:grid ${source.invoiceStyle === "Professional" ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-500"}`}><span>Item</span><span className="text-right">Qty</span><span className="text-right">Unit price</span><span className="text-right">Amount</span></div>}
        {source.lineItems.map((item) => <div key={item.id} className={`grid gap-2 px-4 py-4 sm:grid-cols-[minmax(0,1fr)_70px_130px_130px] sm:gap-3 ${source.invoiceStyle === "Creative" ? "rounded-2xl bg-primary/6" : "border-t border-slate-200 first:border-t-0"}`}><div><p className="break-words font-semibold">{item.item}</p>{item.description && <p className="mt-1 break-words text-xs leading-5 text-slate-500">{item.description}</p>}</div><p className="text-sm sm:text-right"><span className="sm:hidden">Quantity: </span>{item.quantity}</p><p className="text-sm sm:text-right"><span className="sm:hidden">Unit price: </span>{formatRupiah(item.unitPrice)}</p><p className="font-semibold sm:text-right">{formatRupiah(item.quantity * item.unitPrice)}</p></div>)}
      </div>
      <div className="ml-auto mt-6 max-w-sm space-y-2 text-sm"><div className="flex justify-between gap-5"><span className="text-slate-500">Subtotal</span><span>{formatRupiah(totals.subtotal)}</span></div>{totals.discount > 0 && <div className="flex justify-between gap-5"><span className="text-slate-500">Discount{source.discountMode === "percentage" ? ` (${source.discountValue}%)` : ""}</span><span>-{formatRupiah(totals.discount)}</span></div>}{totals.tax > 0 && <div className="flex justify-between gap-5"><span className="text-slate-500">{invoiceTaxLabel(source)}</span><span>{source.taxTreatment === "deducted" ? "-" : ""}{formatRupiah(totals.tax)}</span></div>}<div className="flex justify-between gap-5 border-t border-slate-200 pt-3 text-lg font-bold"><span>Total</span><span>{formatRupiah(totals.total)}</span></div><div className="flex justify-between gap-5"><span className="text-slate-500">Paid</span><span>{formatRupiah(paid)}</span></div><div className="flex justify-between gap-5 text-base font-bold text-primary"><span>Remaining</span><span>{formatRupiah(remaining)}</span></div>{paid > totals.total && <div className="flex justify-between gap-5 text-sm font-semibold text-amber-700"><span>Overpaid</span><span>{formatRupiah(paid - totals.total)}</span></div>}</div>
      ...
      {(source.signatureImage || source.stampImage) && <section className="mt-8 border-t border-slate-200 pt-6"><p className="text-xs font-bold uppercase tracking-wider text-slate-600">Authorized signature</p><div className="relative mt-2 h-32 w-56">{source.signatureImage && <img src={source.signatureImage} alt="Visual business signature" className="absolute bottom-2 left-0 h-20 max-w-48 object-contain object-left" />}{source.stampImage && <img src={source.stampImage} alt="Business stamp" className="absolute bottom-0 left-12 z-10 h-24 w-24 object-contain" />}</div></section>}
    </div>
...
function InvoiceEditor({ invoice, customers, onClose, onSave }: { invoice: Invoice | null; customers: Array<{ id: string; name: string; phone: string; email: string }>; onClose: () => void; onSave: (invoice: Invoice) => void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[calc(100dvh-1rem)] max-w-[calc(100%-1rem)] sm:max-w-3xl"><DialogHeader><DialogTitle>{draft.bookingId ? "Booking invoice" : "Custom invoice"}</DialogTitle><DialogDescription>{draft.bookingId ? "Booking details are prefilled. Changes here do not change the booking." : "A custom invoice does not create a Booking, Payment, or Income record."}</DialogDescription></DialogHeader><div className="space-y-6">
    ...
    <section className="rounded-2xl bg-muted p-4 sm:p-5"><h3 className="font-semibold">Summary</h3><div className="mt-4 space-y-2"><div className="flex justify-between text-sm"><span>Subtotal</span><span>{formatRupiah(totals.subtotal)}</span></div>{totals.discount > 0 && <div className="flex justify-between text-sm"><span>Discount{draft.discountMode === "percentage" ? ` (${draft.discountValue}%)` : ""}</span><span>-{formatRupiah(totals.discount)}</span></div>}{totals.tax > 0 && <div className="flex justify-between text-sm"><span>{invoiceTaxLabel(draft)}</span><span>{draft.taxTreatment === "deducted" ? "-" : ""}{formatRupiah(totals.tax)}</span></div>}<div className="mt-3 flex justify-between border-t border-border pt-3 text-lg font-bold"><span>Total</span><span>{formatRupiah(totals.total)}</span></div></div></section>
