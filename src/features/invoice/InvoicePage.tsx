...
function customDraft(settings: InvoiceSettings, client: { id: string; name: string; phone: string; email: string } | undefined, timezone: string): Invoice {
  const now = Date.now(); const date = todayKey(timezone);
...
function paymentLabel(invoice: Invoice, payments: ReturnType<typeof usePayments>["payments"], currentTodayKey: string) {
  const source = invoice.snapshot ?? invoice;
  return invoicePaymentStatus(invoiceTotals(source).total, invoicePaidAmount(invoice, payments), source.dueDate, currentTodayKey);
}

function InvoiceStatus({ invoice, payments, currentTodayKey }: { invoice: Invoice; payments: ReturnType<typeof usePayments>["payments"]; currentTodayKey: string }) {
  const payment = paymentLabel(invoice, payments, currentTodayKey);
  return <span className="inline-flex flex-wrap items-center gap-1.5"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${invoice.lifecycle === "Issued" ? "bg-accent text-accent-foreground" : "bg-muted text-muted-foreground"}`}>{invoice.lifecycle}</span><span className="text-xs text-muted-foreground">·</span><span className={`text-xs font-semibold ${payment === "Overdue" ? "text-rose-700" : ""}`}>{payment}</span></span>;
}
...
export default function InvoicePage() {
  ...
  const rows = useMemo(() => {
    ...
    const visible = latestInvoiceVersions(invoices).filter((invoice) => (filter === "All" || (filter === "Draft" ? invoice.lifecycle !== "Issued" : paymentLabel(invoice, paymentData.payments, currentTodayKey) === filter)) && (!keyword || [invoice.invoiceNumber, invoice.clientName, invoice.serviceName].filter(Boolean).join(" ").toLowerCase().includes(keyword)));
    ...
    const status = (invoice: Invoice) => `${invoice.lifecycle} ${paymentLabel(invoice, paymentData.payments, currentTodayKey)}`;
    switch (sort) {
  ...
  }, [invoices, filter, search, sort, paymentData.payments, currentTodayKey]);
