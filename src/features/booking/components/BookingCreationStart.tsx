...
const STORAGE_KEY = "qai:booking-client-template";
const FIELD_LABELS: Record<BookingTemplateField, string> = {
  name: "Name", phone: "Phone", instagram: "Instagram", email: "Email", service: "Service",
  date: "Date", startTime: "Start time", endTime: "End time", location: "Location", notes: "Notes",
};

function loadPreferences(): BookingTemplatePreferences {
  if (typeof window === "undefined") return structuredClone(DEFAULT_BOOKING_TEMPLATE_PREFERENCES);
  try {
    const value = JSON.parse(window.localStorage.getItem(STORAGE_KEY) ?? "null") as Partial<BookingTemplatePreferences> | null;
    if (!value) return structuredClone(DEFAULT_BOOKING_TEMPLATE_PREFERENCES);
    const enabled = (value.enabledFields ?? []).filter((field, index, fields): field is BookingTemplateField => BOOKING_TEMPLATE_FIELDS.includes(field as BookingTemplateField) && fields.indexOf(field) === index);
    return {
      introduction: typeof value.introduction === "string" ? value.introduction : DEFAULT_BOOKING_TEMPLATE_PREFERENCES.introduction,
      closing: typeof value.closing === "string" ? value.closing : DEFAULT_BOOKING_TEMPLATE_PREFERENCES.closing,
      enabledFields: enabled.length ? enabled : [...DEFAULT_BOOKING_TEMPLATE_PREFERENCES.enabledFields],
    };
  } catch {
    return structuredClone(DEFAULT_BOOKING_TEMPLATE_PREFERENCES);
  }
}
...
export default function BookingCreationStart({ mode, customers, services, onModeChange, onReview }: BookingCreationStartProps) {
  ...
  const [preferences, setPreferences] = useState<BookingTemplatePreferences>(loadPreferences);
  ...
  const orderedFields = [...preferences.enabledFields, ...BOOKING_TEMPLATE_FIELDS.filter((field) => !preferences.enabledFields.includes(field))];

  function parse() {
    if (!source.trim()) return notify.error("Paste the completed booking text first.");
    const result = parseBookingText(source);
  ...
  function savePreferences(next: BookingTemplatePreferences) {
    setPreferences(next);
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
  }

  function moveField(field: BookingTemplateField, direction: -1 | 1) {
    const fields = [...preferences.enabledFields];
    const index = fields.indexOf(field);
    const target = index + direction;
    if (index < 0 || target < 0 || target >= fields.length) return;
    [fields[index], fields[target]] = [fields[target], fields[index]];
    savePreferences({ ...preferences, enabledFields: fields });
  }
  ...
  if (mode === "template") {
    return <div className="grid min-w-0 gap-5 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="space-y-5 rounded-2xl border border-border bg-muted/25 p-4 sm:p-5">
        ...
        <fieldset><legend className="text-sm font-semibold">Fields and order</legend><div className="mt-2 space-y-2">{orderedFields.map((field) => { const index = preferences.enabledFields.indexOf(field); const enabled = index >= 0; return <div key={field} className="flex min-h-11 items-center gap-2 rounded-lg border border-border bg-card px-3 text-sm"><label className="flex min-w-0 flex-1 items-center gap-2"><input type="checkbox" checked={enabled} onChange={(event) => savePreferences({ ...preferences, enabledFields: event.target.checked ? [...preferences.enabledFields, field] : preferences.enabledFields.filter((item) => item !== field) })} /><span className="truncate">{FIELD_LABELS[field]}</span></label>{enabled && <><Button type="button" size="icon-sm" variant="ghost" aria-label={`Move ${FIELD_LABELS[field]} earlier`} disabled={index === 0} onClick={() => moveField(field, -1)}><ArrowUp className="size-4" /></Button><Button type="button" size="icon-sm" variant="ghost" aria-label={`Move ${FIELD_LABELS[field]} later`} disabled={index === preferences.enabledFields.length - 1} onClick={() => moveField(field, 1)}><ArrowDown className="size-4" /></Button></>}</div>; })}</div></fieldset>
