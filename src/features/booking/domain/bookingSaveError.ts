...
function safeBookingSaveReason(error: unknown, status?: number | null): string {
  ...
  if (code === "BOOKING_PAYMENT_MIGRATION_REQUIRED") {
  ...
  if (/payment exceeds the booking outstanding amount/.test(text)) {
    return "The initial payment exceeds the booking outstanding amount.";
  }
  if (/new payments cannot be added to a cancelled booking/.test(text)) {
    return "An initial payment cannot be added to a cancelled booking.";
  }
  if (/invalid initial payment|initial payment booking mismatch/.test(text)) {
    return "The initial payment details are invalid.";
  }
