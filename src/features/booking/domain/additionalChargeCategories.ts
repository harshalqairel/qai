...
const DEFAULT_NAMES = ["Transportation", "Parking", "Toll", "Accommodation", "Overtime", "Extra person", "Extra assistant", "Other"];
...
export function getAdditionalChargeCategories(): AdditionalChargeCategory[] {
  ...
  return DEFAULT_NAMES.map((name, index) => ({ id: `charge-category-${index + 1}`, name, createdAt: 0 }));
}
...
export async function loadAdditionalChargeCategories(): Promise<AdditionalChargeCategory[]> {
  ...
  return categories.length ? categories : getAdditionalChargeCategories();
}
