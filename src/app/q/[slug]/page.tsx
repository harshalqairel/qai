...
function newAvailabilitySchedule(): PublicSchedule {
  return { id: crypto.randomUUID(), label: "", date: "", startTime: "", endTime: "", location: "" };
}
...
function initialForm(service: PublicService, variantId: string | null): FormState {
  ...
  return {
    ...
    instagram: "",
    need: "",
    schedules: Array.from({ length: count }, () => normalizeServiceAvailability(service.availability).mode === "Flexible" ? newSchedule(snapshot.duration) : newAvailabilitySchedule()),
...
function AvailabilityScheduleFields({ page, service, variantId, snapshotCount, form, change, updateSchedule }: { page: QaiPageConfig; service: PublicService; variantId: string | null; snapshotCount: number; form: FormState; change: (changes: Partial<FormState>) => void; updateSchedule: (id: string, changes: Partial<PublicSchedule>) => void }) {
  const availability = normalizeServiceAvailability(service.availability);
  const [slotsBySchedule, setSlotsBySchedule] = useState<Record<string, PublicServiceSlotAvailability[]>>({});
  const [loadingScheduleId, setLoadingScheduleId] = useState<string | null>(null);
  const datedDates = [...new Set(availability.datedSessions.filter((slot) => slot.active).map((slot) => slot.date))].sort();

  async function chooseDate(schedule: PublicSchedule, date: string) {
    updateSchedule(schedule.id, { date, startTime: "", endTime: "", location: "" });
    if (!date) {
      setSlotsBySchedule((current) => ({ ...current, [schedule.id]: [] }));
      return;
    }
  ...
  return <section><div className="flex flex-wrap items-end justify-between gap-3"><div><SectionTitle>Choose schedule</SectionTitle><p className="mt-1 text-sm text-muted-foreground">Select from the times offered by {page.businessName}.</p></div><Button type="button" variant="outline" size="sm" disabled={form.schedules.length >= 12} onClick={() => change({ schedules: [...form.schedules, newAvailabilitySchedule()] })}><Plus className="size-4" />Add schedule</Button></div><div className="mt-4 space-y-3">{form.schedules.map((schedule, index) => { const slots = slotsBySchedule[schedule.id] ?? []; return <article key={schedule.id} className="rounded-xl border border-border bg-card p-4"><div className="flex items-center justify-between gap-3"><p className="font-semibold">Schedule {index + 1}</p>{form.schedules.length > snapshotCount && <Button type="button" variant="ghost" size="sm" onClick={() => change({ schedules: form.schedules.filter((item) => item.id !== schedule.id) })}><X className="size-4" />Remove</Button>}</div><div className="mt-4"><Field label="Date">{availability.mode === "Dated sessions" ? <select className="h-11 w-full rounded-lg border border-input bg-background px-3 text-sm" value={schedule.date} onChange={(event) => void chooseDate(schedule, event.target.value)}><option value="">Select a date</option>{datedDates.map((date) => <option key={date} value={date}>{new Date(`${date}T12:00:00.000Z`).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", timeZone: page.timezone })}</option>)}</select> : <Input type="date" value={schedule.date} onChange={(event) => void chooseDate(schedule, event.target.value)} />}</Field></div>{loadingScheduleId === schedule.id ? <p className="mt-4 text-sm text-muted-foreground">Checking available times…</p> : schedule.date && slots.length === 0 ? <p className="mt-4 rounded-lg bg-muted p-3 text-sm text-muted-foreground">No available times on this date. Choose another date.</p> : slots.length > 0 ? <fieldset className="mt-4"><legend className="text-sm font-semibold">Available time</legend><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">{slots.map((slot) => { const selected = schedule.startTime === slot.startTime; const full = slot.remaining <= 0; return <button key={slot.key} type="button" disabled={full} aria-pressed={selected} onClick={() => updateSchedule(schedule.id, { startTime: slot.startTime, endTime: slot.endTime, location: slot.location })} className="min-h-12 rounded-xl border border-border px-3 py-2 text-left text-sm transition aria-pressed:border-primary aria-pressed:bg-primary/8 disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground"><span className="block font-semibold">{slot.startTime}</span><span className="mt-0.5 block text-xs">{full ? "Full" : slot.capacity > 1 ? `${slot.remaining} ${slot.remaining === 1 ? "spot" : "spots"} left` : "Available"}</span></button>; })}</div></fieldset> : null}</article>; })}</div></section>;
}
...
function Confirmation({ request, page, onClose }: { request: PublicRequest; page: QaiPageConfig; onClose: () => void }) {
  ...
  return <div className="overflow-y-auto p-6 text-center sm:p-10"><CheckCircle2 className="mx-auto size-12 text-emerald-600" /><h2 className="mt-5 text-2xl font-bold">{request.type === "Instant booking" ? "Booking confirmed." : request.type === "Inquiry" ? "Inquiry sent." : "Booking sent."}</h2>{request.type === "Instant booking" && schedule ? <div className="mx-auto mt-5 max-w-md rounded-xl bg-muted p-4"><p className="font-semibold">{request.serviceName}</p><p className="mt-1 text-sm text-muted-foreground">{new Date(`${schedule.date}T00:00:00`).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })} · {schedule.startTime}</p>{canShowBookedThroughQai(request) && <p className="mt-4 text-xs text-muted-foreground">Booked through Qai</p>}</div> : <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">{request.type === "Inquiry" ? `${page.businessName} will get back to you.` : `Your booking has been sent for confirmation. ${page.businessName} will let you know once it is confirmed.`}</p>}<Button className="mt-6" onClick={onClose}>Back to page</Button><div className="mt-8"><PoweredByQai /></div></div>;
}
