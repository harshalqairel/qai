...
function initialForm(service: Pick<PublicService, "actionMode" | "defaultSessionCount" | "durationMinutes">): FormState { const count = service.actionMode === "Booking request" ? service.defaultSessionCount : 0; return { clientName: "", whatsapp: "", email: "", need: "", schedules: Array.from({ length: count }, () => newSchedule(service as PublicService)), location: "", budget: "", notes: "", instantSlotId: null }; }
...
export default function PublicQaiPage() {
  ...
  const [selected, setSelected] = useState<PublicService | null>(null); const [form, setForm] = useState<FormState>(() => initialForm({ actionMode: "Booking request", defaultSessionCount: 1, durationMinutes: 60 }));
  ...
  async function submit() {
    ...
    try {
      const created = await validationClient.submitRequest({ pageId: page.id, slug: page.slug, serviceId: selected.serviceId, serviceName: selected.title, type: selected.actionMode,
        clientName: form.clientName, whatsapp: form.whatsapp, email: form.email, need: form.need, schedules: form.schedules.map((item) => ({ ...item, endTime: derivePublicEndTime(item.startTime, selected.durationMinutes) })), location: form.location,
