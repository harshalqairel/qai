...
import { normalizeQaiSpaceTab, QAI_SPACE_TABS, qaiSpaceHref, type QaiSpaceTab } from "@/lib/qaiSpaceRouting";
import {
  defaultQaiPage,
  normalizeSlug,
  publicActionLabel,
...
function QaiPageOwnerContent() {
  ...
  return <main className="min-h-screen"><div className="page-shell">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="page-title">Qai Space</h1><p className="mt-2 text-sm text-muted-foreground sm:text-base">Your public home for services, portfolio, and bookings.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => void copyLink()}><Clipboard className="size-4" /> Copy link</Button><Button onClick={() => void savePage()} disabled={saving}><Save className="size-4" /> {saving ? "Publishing…" : "Publish changes"}</Button></div></header>
    <nav className="grid grid-cols-4 gap-1 rounded-xl border border-border bg-card p-1 sm:flex" aria-label="Qai Space sections">{QAI_SPACE_TABS.map((item) => <button key={item} type="button" onClick={() => selectTab(item)} className={`min-h-11 min-w-0 rounded-lg px-1 text-[11px] font-semibold sm:px-4 sm:text-sm ${tab === item ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:bg-muted"}`}>{item}{item === "Requests" && pendingCount > 0 ? ` (${pendingCount})` : ""}</button>)}</nav>
