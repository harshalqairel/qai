...
import { normalizeQaiSpaceTab, QAI_SPACE_OWNER_STEPS, qaiSpaceHref, type QaiSpaceTab } from "@/lib/qaiSpaceRouting";
import {
  defaultQaiPage,
  normalizeSlug,
  publicActionLabel,
...
async function readImage(file: File | undefined, kind: "page-logo" | "page-cover" | "portfolio", onLoad: (value: string) => void) {
  ...
  if (!isValidationModeEnabled() && !isCloudModeEnabled()) { const reader = new FileReader(); reader.onload = () => onLoad(String(reader.result ?? "")); reader.readAsDataURL(file); return; }
  try { const uploaded = await qaiSpaceClient.uploadMedia(file, kind); onLoad(uploaded.url); } catch (error) { notify.error(error instanceof Error ? error.message : "Could not upload that image."); }
}
...
function QaiPageOwnerContent() {
  ...
  const tab = normalizeQaiSpaceTab(searchParams.get("tab"));
  ...
  const load = useCallback(async (refreshPage = true) => {
    try { const [store, questionnaire] = await Promise.all([qaiSpaceClient.owner(), loadBookingQuestionnaire()]); const existing = store.pages[0]; if (refreshPage) setPage({ ...(existing ?? defaultQaiPage()), questionnaire }); setRequests(store.requests.filter((item) => !existing || item.pageId === existing.id).sort((a, b) => b.submittedAt - a.submittedAt)); setBackendCapabilities(store.capabilities); }
    catch (error) { notify.error(error instanceof Error ? error.message : "Could not load Qai Space data."); }
  ...
  function selectTab(nextTab: QaiSpaceTab) {
    const params = new URLSearchParams(searchParams.toString());
    params.set("tab", nextTab);
    router.replace(`${qaiSpaceHref()}?${params.toString()}`, { scroll: false });
  }
  ...
  async function removePortfolio(id: string, imageUrl: string) { setPage((current) => ({ ...current, portfolio: current.portfolio.filter((item) => item.id !== id).map((item, position) => ({ ...item, position })) })); if (!isValidationModeEnabled() && !isCloudModeEnabled()) return; try { await qaiSpaceClient.deleteMedia(imageUrl); } catch { notify.error("The item was removed from this draft, but its image could not be deleted."); } }
  async function savePage() {
    const slug = normalizeSlug(page.slug); if (!slug) return notify.error("Enter a Space address using letters or numbers.");
    if (tiktokInvalid) return notify.error("Enter a TikTok username or profile link.");
    setSaving(true); try { const saved = await qaiSpaceClient.savePage({ ...page, slug, services: configuredServices, timezone: bookingData.timezone, updatedAt: Date.now() }); setPage(saved); notify.success("Qai Space saved."); }
    catch (error) { notify.error(error instanceof Error ? error.message : "Could not save Qai Space."); } finally { setSaving(false); }
  }
  async function copyLink() { const url = `${window.location.origin}/q/${page.slug}`; await navigator.clipboard.writeText(url); notify.success("Space link copied."); }
  function addSlot() {
  ...
  async function ensureClient(request: PublicRequest) {
    ...
    return exact ?? customerData.createCustomerAndReturn({ name: request.clientName, phone: request.whatsapp, email: request.email, instagram: request.instagram, notes: "Created from a Qai Space booking request." });
  }
  ...
  async function accept(request: PublicRequest) {
    ...
    try {
      await convertRequestToBooking({
        ...
        clientId: client.id,
        claimRequest: qaiSpaceClient.claimRequest,
        createBooking: (requestId) => bookingData.createBookingAndReturnOrThrow({ requestId, booking: values, initialPayment: null }),
        linkRequest: (requestId, bookingId, clientId) => qaiSpaceClient.updateRequest(requestId, "Accepted", bookingId, clientId),
        releaseClaim: (requestId, clientId) => qaiSpaceClient.updateRequest(requestId, "Pending", null, clientId),
      });
      await load();
  ...
  async function confirmDecline(openWhatsApp: boolean) {
    ...
    try {
      await qaiSpaceClient.updateRequest(declineTarget.id, "Declined", null, declineTarget.clientId);
  ...
  return <main className="min-h-screen"><div className="page-shell">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="page-title">Qai Space</h1><p className="mt-2 text-sm text-muted-foreground sm:text-base">Build, preview, and publish your public business page.</p></div><div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"><Button variant="outline" onClick={() => void copyLink()}><Clipboard className="size-4" /> Copy link</Button><Button onClick={() => void savePage()} disabled={saving}><Save className="size-4" /> {saving ? "Publishing…" : "Publish changes"}</Button></div></header>
    <nav className="-mx-4 flex gap-1 overflow-x-auto border-y border-border bg-card px-4 py-1.5 sm:mx-0 sm:rounded-xl sm:border sm:p-1" aria-label="Qai Space setup steps">{QAI_SPACE_OWNER_STEPS.map((item) => <button key={item.tab} type="button" onClick={() => selectTab(item.tab)} className={`min-h-11 shrink-0 rounded-lg px-3 text-sm font-semibold sm:px-4 ${tab === item.tab ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:bg-muted"}`}>{item.label}{item.tab === "Requests" && pendingCount > 0 ? ` (${pendingCount})` : ""}</button>)}</nav>
    ...
    {tab === "Profile" && <section className="surface-card p-5 sm:p-6"><h2 className="section-title">Public profile</h2><p className="mt-1 text-sm text-muted-foreground">Keep your profile short and easy for customers to understand.</p><div className="mt-6 grid gap-5 sm:grid-cols-2"><Field label="Business logo/photo"><div className="flex items-center gap-3">{page.logo && <img src={page.logo} alt="Business logo preview" className="size-16 rounded-xl border border-border object-cover" />}<Input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => void readImage(event.target.files?.[0], "page-logo", (logo) => setPage((current) => ({ ...current, logo })))} /></div></Field><Field label="Cover image"><Input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => void readImage(event.target.files?.[0], "page-cover", (coverImage) => setPage((current) => ({ ...current, coverImage })))} /></Field><Field label="Business name"><Input value={page.businessName} onChange={(event) => setPage({ ...page, businessName: event.target.value })} /></Field><Field label="Space address"><div className="flex items-center rounded-lg border border-border bg-card pl-3 text-sm text-muted-foreground"><span>/q/</span><Input className="border-0 shadow-none focus-visible:ring-0" value={page.slug} onChange={(event) => setPage({ ...page, slug: normalizeSlug(event.target.value) })} /></div></Field><div className="sm:col-span-2"><Field label="Short description"><Textarea rows={3} value={page.shortDescription} onChange={(event) => setPage({ ...page, shortDescription: event.target.value })} /></Field></div><Field label="Location"><Input value={page.location} onChange={(event) => setPage({ ...page, location: event.target.value })} /></Field><Field label="WhatsApp"><Input inputMode="tel" value={page.whatsapp} onChange={(event) => setPage({ ...page, whatsapp: event.target.value })} /></Field><Field label="Email"><Input type="email" value={page.email} onChange={(event) => setPage({ ...page, email: event.target.value })} /></Field><Field label="Instagram"><Input aria-label="Instagram" value={page.instagram} placeholder="@username or profile link" onChange={(event) => setPage({ ...page, instagram: event.target.value })} /></Field><div className="sm:col-span-2"><Field label="TikTok"><Input aria-label="TikTok" value={page.tiktok} placeholder="@username or profile link" aria-invalid={tiktokInvalid} aria-describedby={tiktokInvalid ? "qai-page-tiktok-error" : undefined} onChange={(event) => setPage({ ...page, tiktok: event.target.value })} /></Field>{tiktokInvalid && <p id="qai-page-tiktok-error" className="mt-1.5 text-sm font-medium text-destructive">Enter a TikTok username or profile link.</p>}</div></div><p className="mt-5 rounded-lg bg-muted p-3 text-xs text-muted-foreground">{isValidationModeEnabled() ? "Your public profile, services, and selected work are saved to this remote validation workspace." : isCloudModeEnabled() ? "Your published profile and media are saved securely to your business cloud workspace." : "Your public profile, services, and selected work are saved in this browser."}</p></section>}
    ...
    {tab === "Requests" && <BookingRequestList requests={requests} filter={requestFilter} onFilterChange={setRequestFilter} onRefresh={() => void load()} onAccept={(request) => void accept(request)} onReview={(request) => void editAndAccept(request)} onDecline={decline} focusRequestId={searchParams.get("request")} />}
    {tab === "Preview" && <section className="surface-card overflow-hidden"><div className="flex flex-col gap-4 border-b border-border p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><h2 className="section-title">Public preview</h2><p className="mt-1 text-sm text-muted-foreground">This uses the same renderer as your published Space.</p></div><div className="flex flex-wrap gap-2"><div className="flex rounded-lg border border-border p-1">{(["Desktop", "Mobile"] as const).map((device) => <button type="button" key={device} aria-pressed={previewDevice === device} onClick={() => setPreviewDevice(device)} className={`min-h-9 rounded-md px-3 text-sm font-semibold ${previewDevice === device ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}>{device}</button>)}</div><Button render={<Link href={`/q/${page.slug}`} target="_blank" />}><Eye className="size-4" /> View Space</Button></div></div><div className="max-h-[70rem] overflow-y-auto bg-muted p-0 sm:p-4"><div className={`mx-auto overflow-hidden bg-white transition-[max-width] ${previewDevice === "Mobile" ? "max-w-[390px] shadow-xl" : "max-w-none"}`}><QaiPageRenderer page={{ ...page, services: configuredServices }} services={configuredServices.filter((item) => item.visible)} portfolio={[...page.portfolio].filter((item) => item.visible).sort((a, b) => a.position - b.position)} preview /></div></div></section>}
    </div>
    {tab !== "Requests" && tab !== "Preview" && <aside className="hidden min-w-0 xl:sticky xl:top-6 xl:block"><section className="surface-card overflow-hidden"><div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3"><div><p className="section-kicker">Live preview</p><p className="mt-1 text-sm font-semibold">Same renderer as your published Space</p></div><Button type="button" size="sm" variant="outline" onClick={() => selectTab("Preview")}><Eye className="size-4" /> Full preview</Button></div><div className="max-h-[calc(100dvh-10rem)] overflow-y-auto bg-muted p-3"><div className="overflow-hidden bg-white shadow-sm"><QaiPageRenderer page={{ ...page, services: configuredServices }} services={configuredServices.filter((item) => item.visible)} portfolio={[...page.portfolio].filter((item) => item.visible).sort((a, b) => a.position - b.position)} preview /></div></div></section></aside>}
    </div>
  </div>
