...
import { ArrowDown, ArrowUp, Check, Clipboard, Eye, ImagePlus, MessageCircle, Plus, RefreshCw, Save, Trash2, X } from "lucide-react";
...
type Tab = "Page" | "Design" | "Portfolio" | "Services" | "Booking" | "Requests" | "Preview";
...
export default function QaiPageOwner() {
  ...
  function movePortfolio(id: string, direction: -1 | 1) { setPage((current) => { const items = [...current.portfolio].sort((a, b) => a.position - b.position); const index = items.findIndex((item) => item.id === id); const target = index + direction; if (index < 0 || target < 0 || target >= items.length) return current; [items[index], items[target]] = [items[target], items[index]]; return { ...current, portfolio: items.map((item, position) => ({ ...item, position })) }; }); }
  ...
  return <main className="min-h-screen"><div className="page-shell">
    ...
    <nav className="flex gap-1 overflow-x-auto rounded-xl border border-border bg-card p-1" aria-label="Qai Page sections">{(["Page", "Design", "Portfolio", "Services", "Booking", "Requests", "Preview"] as Tab[]).map((item) => <button key={item} type="button" onClick={() => setTab(item)} className={`min-h-11 shrink-0 rounded-lg px-4 text-sm font-semibold ${tab === item ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:bg-muted"}`}>{item}{item === "Requests" && pendingCount > 0 ? ` (${pendingCount})` : ""}</button>)}</nav>
    {tab === "Portfolio" && <section className="surface-card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><h2 className="section-title">Portfolio capacity</h2><p className="mt-1 text-sm text-muted-foreground">{page.portfolio.length} of 40 images · mixed portrait, landscape, and square images are preserved.</p></div><label className={`inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-lg border border-border bg-card px-4 text-sm font-semibold ${page.portfolio.length >= 40 ? "pointer-events-none opacity-50" : "hover:bg-muted"}`}><ImagePlus className="size-4" />Add image<input className="sr-only" type="file" accept="image/png,image/jpeg,image/webp" disabled={page.portfolio.length >= 40} onChange={(event) => void readImage(event.target.files?.[0], "portfolio", (imageUrl) => setPage((current) => ({ ...current, portfolio: [...current.portfolio, { id: crypto.randomUUID(), imageUrl, caption: "", serviceId: null, visible: true, position: current.portfolio.length }] })))} /></label></section>}
    ...
    {tab === "Design" && <section className="surface-card p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="section-title">Complete theme</h2><p className="mt-1 text-sm text-muted-foreground">Controlled colors keep every template readable and consistent.</p></div><Button type="button" variant="outline" size="sm" onClick={() => setPage((current) => ({ ...current, style: defaultQaiPage().style }))}>Reset theme</Button></div>
      <div className="mt-5 flex flex-wrap gap-2">
        <Button type="button" variant="outline" size="sm" onClick={() => setPage((current) => ({ ...current, style: { ...current.style, backgroundColor: "#F7F4F1", surfaceColor: "#FFFFFF", textColor: "#101828", mutedTextColor: "#667085", accentColor: "#8A4B67", buttonBackgroundColor: "#56334F", buttonTextColor: "#FFFFFF", borderColor: "#DED7D2" } }))}>Plum studio</Button>
        <Button type="button" variant="outline" size="sm" onClick={() => setPage((current) => ({ ...current, style: { ...current.style, backgroundColor: "#F5F7FB", surfaceColor: "#FFFFFF", textColor: "#0F1B33", mutedTextColor: "#61708A", accentColor: "#4F6BFF", buttonBackgroundColor: "#233B74", buttonTextColor: "#FFFFFF", borderColor: "#D9E0EB" } }))}>Qai blue</Button>
        <Button type="button" variant="outline" size="sm" onClick={() => setPage((current) => ({ ...current, style: { ...current.style, backgroundColor: "#F5F1EA", surfaceColor: "#FFFCF7", textColor: "#241C18", mutedTextColor: "#70635C", accentColor: "#A55D43", buttonBackgroundColor: "#603E32", buttonTextColor: "#FFFFFF", borderColor: "#DED2C7" } }))}>Warm neutral</Button>
      </div>
